...
export function PortfolioLoader({ isReady, onFinish }: PortfolioLoaderProps) {
  ...
  const loadingText = useMemo(() => `Loading${'.'.repeat(dotCount)}`, [dotCount]);
  ...
  useEffect(() => {
    if (!isReady || sequenceStartedRef.current) return;
  ...
  return (
    <div className={`portfolio-loader${isFading ? ' is-fading' : ''}`} aria-hidden="true">
      ...
      {showReveal && <div className="light-reveal" />}
    </div>
  );
}
